...
parser.add_argument("--task-dir", required=True, help="Path to task directory under configs.2")
...
task_vars_file = os.path.join(task_dir, "task-vars.yaml")
task_devices_file = os.path.join(task_dir, "task-devices.yaml")
...
# Generate Configs
# -------------------------
print(f"[INFO] Generating configs for SP: {sp_name}, task-dir: {task_dir}, template: {template_name}")

for node in sp_data:
    if allowed_devices and node["name"] not in allowed_devices:
        print(f"[DEBUG] Skipping {node['name']} (not in task-devices.yaml)")
        continue  # Skip devices not in task-devices.yaml
