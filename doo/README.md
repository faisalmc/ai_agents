...
This repo contains automation scripts to configure Cisco devices and collect verification outputs as part of task-based workflows (e.g., Task-1, Task-2B)
...
├── task-1/                   # Folder for a specific task
...
Pushes configuration commands from `task-x/*.txt` files into routers listed in `devices.yaml`.

### Usage:

```bash
python3 push_cli_configs.py --task task-1
```
...
- Logs the output per device to: `task-x/logs/{device}.log`
...
### Usage-1: for all devices in `task-x` directory

```bash
python3 run_show_commands.py --task task-1
```
...
- Saves each device's output to: `task-x/show_logs/{device}.log`
...
|----------------------|----------------------------------|-----------------------------|
| `push_cli_configs.py`| Push config to devices           | `task-x/logs/{device}.log`  |
| `run_show_commands.py`| Collect show command output      | `task-x/show_logs/{device}.log` |
