...
SLACK_CHANNEL = os.getenv("SLACK_CHANNEL_ID", "ai-agents")
...
def post_operational_summary(task_name, config_dir, channel_id):
    debug(f"[INFO] post_operational_summary(): arg channel_id = {repr(channel_id)}")
    debug(f"[INFO] env SLACK_CHANNEL_ID = {os.getenv('SLACK_CHANNEL_ID')}")
    debug(f"[INFO] env SLACK_CHANNEL    = {os.getenv('SLACK_CHANNEL')}")
    # Normalize minimal: prefer arg, fallback to env ID
    if not channel_id:
    ...
    if not channel_id:
        raise RuntimeError("No channel provided. Pass 'channel' or set SLACK_CHANNEL_ID.")

    # Optional: immediate probe so logs tell us *why* Slack rejected it
    try:
        _ = client.conversations_info(channel=channel_id)
        debug(f"[INFO] conversations_info OK for channel {channel_id}")
    except SlackApiError as e:
        debug(f"[WARN] conversations_info failed for {channel_id}: {e.response.get('error')}")
    # --- END: simple debug & sanity checks ---
