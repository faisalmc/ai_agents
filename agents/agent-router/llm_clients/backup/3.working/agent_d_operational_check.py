...
def extract_show_cmds_description(config_dir, task_name):
    ...
    Returns a list of dicts: [{intent, command, protocol}] or [{command}] on fallback.
    """
    import re
    ...
    def _fallback_parse_commands(ini_text_local):
        ...
        And ignores commented lines (# or ;)
        """
        cmds, seen = [], set()
        for raw in ini_text_local.splitlines():
            ...
            if not line or line.startswith("#") or line.startswith(";"):
                continue
            # Try to capture "command = show ..." or plain "show ..."
    ...
    try:
        ...
        try:
            ...
            if m:
                ...
                # As a last resort, try to grab the first [ ... ] or { ... } blob
