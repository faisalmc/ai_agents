...
def handle_operational_analyze(ack, command, respond, logger):
    ...
    for host, md_text in md_pairs:
        ...
        obj = per_device_analyze_with_llm(
            ...
            allow_active=False,
            show_cmds=show_cmds,
            host_facts=facts_index.get(host) # added part of v4 .. agent5_facts.json was not passed earlier!
        )

        obj = _reconcile_findings_with_facts(obj, facts_index.get(host))  # <<< v4
