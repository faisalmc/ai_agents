...
def resolve_slack_channel():
    """
    Resolve Slack channel ID from name. If .env has '#channel',
    strip '#' and find its ID. Fallback to raw name if not found.
    """
    channel_name = SLACK_CHANNEL.lstrip("#")
    try:
        channels = slack.conversations_list(types="public_channel,private_channel")
        channel_id = next(
            (ch["id"] for ch in channels["channels"] if ch["name"] == channel_name),
            None
        )
        if channel_id:
        ...
        dbg(f"Failed to resolve channel ID: {e}")
        return channel_name
...
"""
    ...
    for (config_dir, task_dir), task_changes in tasks.items():
        ...
        try:
            channel_id = resolve_slack_channel()  # Ensure we get the actual channel ID
